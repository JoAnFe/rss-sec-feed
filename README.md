...
- **Threat-intel enrichment**: CVEs are cross-referenced against the full CISA KEV record (not just the id list) and the [FIRST EPSS](https://www.first.org/epss/) exploitation-probability API. Items surface a **RANSOMWARE** badge when a KEV entry is tied to known ransomware campaigns, a **KEV due / overdue** remediation deadline, and an **EPSS %** badge for likely-to-be-exploited CVEs. Confirmed-exploited (KEV), ransomware-linked, and high-EPSS items are floated up in the "Top" sort across every tab.
- **Threat-actor & malware tracking**: a curated alias catalog (~45 actors, ~35 malware families) normalizes vendor naming schemes — Fancy Bear = Sofacy = Forest Blizzard = **APT28** — and tags stories with clickable **ACTOR** / **MALWARE** badges. Search is alias-aware (searching "fancy bear" finds APT28 stories), clicking a badge filters the feed to that name, and any story naming a tracked actor or family joins the Threat Intel tab regardless of source. Names that double as common words ("Play", "Medusa", "Pegasus") require a qualifier so ordinary prose can't trigger them.
- **Actionable view**: prioritises items with operational defensive signals such as CISA KEV membership, ransomware-campaign use, high predicted exploitation (EPSS), active exploitation, critical impact, urgent containment or a concrete remediation path, and explains why each item was selected.
...
- Tabs for **All / Actionable / Bookmarks / Cybersecurity / Frontier AI / Exploited CVEs / Threat Intel**:
  ...
  - **Exploited CVEs** — items mentioning a CVE are tagged (`CVE-YYYY-NNNN` regex) and cross-referenced against the [CISA KEV catalog](https://www.cisa.gov/known-exploited-vulnerabilities-catalog) (full record — ransomware-campaign use, remediation due date, vendor/product — refetched every 6h, cached in `data/kev.json`, failure-tolerant), plus "actively exploited / in the wild / zero-day" keyword signals. EPSS scores for the corpus CVEs are fetched from FIRST (cached in `data/epss.json`, also failure-tolerant).
